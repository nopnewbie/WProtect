...
public:
    void protect_address_disas(const char * _filename,std::map<long,long> & user_protect_address)
    {
        ...
        for (auto iter = user_protect_address.begin(); iter != user_protect_address.end(); iter++)
        {
        ...
        for (auto iter = build_info.begin(); iter != build_info.end(); iter++)
        {
#ifdef WPROTECT_USE_QT
            //error("abc");
    ...
    {
        for (auto iter = addr_table_entry.begin(); iter != addr_table_entry.end();iter++)
        {
